...
#######################################################
class IMPORT_OT_wbl(bpy.types.Operator, ImportHelper):
    bl_idname = "import_scene.wbl_sector"
    bl_label = "Import Worldblock Sectors (.wbl)"
    filename_ext = ".wbl"
    ...
    def execute(self, context):
        ...
        for fp in filepaths:
            self.import_wbl(fp, context)

        return {'FINISHED'}

    def import_wbl(self, filepath, context):
        logf = None
        if DEBUG_MODE:
            logpath = get_debug_logfile(filepath)
        ...
        try:
            ...
            # --- Sectors A to D: Each model has them, and each sector is 12 bytes; start at 0x28
            sector_ofs = 0x28
            ...
            for MeshOffset in sorted(mesh_offsets_found):
                ...
                uvOffset     = struct.unpack_from('<f', file_bytes, MeshOffset+40)[0]
                ...
                for mi in range(numMaterials):
                    ...
                    if vertex_count >= 3:
                        ...
                        for i in range(vertex_count - 2):
                            ...
                            idx1 = i + 1
                            idx2 = i + 2
                            face = (idx0, idx2, idx1) if flip else (idx0, idx1, idx2)
                            if len(set(face)) == 3:
                    ...
                    for face in mesh.polygons:
                        for loop_index in face.loop_indices:
                            ...
                            uv_layer.data[loop_index].uv = part_uvs[vert_index]
                    ...
                    image_path = os.path.join(os.path.dirname(filepath), f"{mat_name}.png")
...
def menu_func_import(self, context):
    self.layout.operator(IMPORT_OT_wbl.bl_idname, text="R* Leeds CW Worldblock(.wbl)")
